...
func main() {
	...
	// Set up cleanup handlers for SSH tunnels
	defer func() {
...
func propagateDockerContextToEnv() {
	...
	if ctx.IsRemote {
		klog.Infof("Using remote Docker context: %s (host: %s)", ctx.Name, ctx.Host)
		if ctx.IsSSH {
			klog.Infof("Docker context uses SSH connection")
		}
	}

	// Handle TLS configuration for remote Docker
	if ctx.IsRemote && !ctx.IsSSH && ctx.TLSData != nil {
		// For TLS connections, we would need to set up certificates
		// This is a placeholder for TLS certificate handling
		klog.Infof("Remote Docker context requires TLS - ensure certificates are properly configured")
	}
}
