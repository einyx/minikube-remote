...
func (d *Driver) GetSSHHostname() (string, error) {
	...
	if oci.IsRemoteDockerContext() {
		...
		// TLS contexts need the actual remote host
		ctx, err := oci.GetCurrentContext()
		if err == nil && ctx.Host != "" {
			u, err := url.Parse(ctx.Host)
			if err == nil && u.Hostname() != "" {
				klog.Infof("GetSSHHostname: Using remote host %s for TLS Docker context", u.Hostname())
				return u.Hostname(), nil
			}
		}
		klog.Warningf("GetSSHHostname: Failed to parse remote host, falling back to localhost")
		return "127.0.0.1", nil
	}
	return oci.DaemonHost(d.DriverName()), nil
}
